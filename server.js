...
// ── CORREÇÃO #4: prefixos url_ nos campos das exchanges ──────
...
async function monitorarLive() {
  try {
    ...
    for (const jogo of jogosLive) {
      ...
      // ── CORREÇÃO #5: detectar mudança de placar e editar mensagens ──
...
// ── Atualizar placar nas mensagens existentes ─────────────────
async function atualizarPlacarNasMensagens(jogo, estado, placarAtual, hoje) {
  ...
  for (const [stratKey, alertasArr] of Object.entries(estado.msgIds || {})) {
    if (!alertasArr?.length) continue;
    const ultimo = alertasArr[alertasArr.length - 1];
    const stratBase = stratKey.replace(/_entrada$|_0x0$|_raio$|_c[123]$|_perd.*$|_atencao.*$/, '');
    const display = STRAT_DISPLAY[stratBase] || stratKey;
    const novoTexto = `${display}\n⚽ <b>${jogo.mandante} x ${jogo.visitante}</b>\n📊 ${placarAtual} · ${tempo}'\n⏰ ${hora}\n🔄 Placar atualizado`;
    await editTelegram(ultimo.ids, novoTexto);
  }
}
...
async function processarAlertasLive(jogo, estado, jogoId, hoje) {
  ...
  // ── CORREÇÃO #1 e #6: isHT correto ──
  const isHT  = jogo.tempo === 'Intervalo';
  const is1T  = (periodo === '1_tempo' || tempo <= 45) && !isHT;
  // CORREÇÃO #1: is2T exclui explicitamente o intervalo
  ...
  // ── Função alertar: 1x por stratKey+placar ──
  async function alertar(stratKey, texto, stratRegistrar = null) {
    const nKey = `${jogoId}_${stratKey}_${placar}`;
  ...
  // 3. GOL NO FINAL — CORREÇÃO #1: !isHT obrigatório
  ...
  // 8. LAY 0x1 — CORREÇÃO #2: reentrada no 2T por placar
  if (pendJogo.some(p => p.strat === 'lay_0x1')) {
    if (raioMand) {
      if (total === 0)
        ...
        await alertar(`lay_0x1_atencao_${placar}`, `🤖 <b>LAY 0x1 — ⚠️ Placar 0x1!</b>\n💡 Raio Mandante — feche e aguarde!`, 'lay_0x1_live');
      // CORREÇÃO #2: reentrada no 2T após qualquer raio mandante com placar diferente de 0x1
      else if (is2T && !(golsCasa === 0 && golsFora === 1))
        await alertar(`lay_0x1_reentrada_${placar}`, `🤖 <b>LAY 0x1 — Reentrada 2T!</b>\n💡 Raio Mandante no 2T · Placar ${placar}`, 'lay_0x1_live');
    }
  }

  // 9. LAY 1x0 — CORREÇÃO #2: reentrada no 2T
  if (pendJogo.some(p => p.strat === 'lay_1x0')) {
    if (raioVisit) {
      if (total === 0)
        ...
        await alertar(`lay_1x0_atencao_${placar}`, `🤖 <b>LAY 1x0 — ⚠️ Placar 1x0!</b>\n💡 Raio Visitante — feche e aguarde!`, 'lay_1x0_live');
      else if (is2T && !(golsCasa === 1 && golsFora === 0))
        await alertar(`lay_1x0_reentrada_${placar}`, `🤖 <b>LAY 1x0 — Reentrada 2T!</b>\n💡 Raio Visitante no 2T · Placar ${placar}`, 'lay_1x0_live');
    }
  }
  ...
  // 12. CORREÇÃO LAY FAVORITO — CORREÇÃO #6: !isHT
  ...
  // 13. CORREÇÃO LAY ZEBRA — CORREÇÃO #6: !isHT
  ...
  // OVER 0,5 GONZA (bolinha verde) — CORREÇÃO #3: raioMand E raioVisit no 1T
  if (pendJogo.some(p => p.strat === 'over05')) {
    // 1T + 0x0 + raio mandante E visitante → alerta Over HT
    if (is1T && total === 0 && raioMand && raioVisit)
      await alertar('over05_ht_dois_raios', `🟢 <b>OVER 0.5 — Entrar Over HT!</b>\n💡 0x0 + Raio dos dois times no 1T!`, 'over05_live');
    // HT 0x0 com raio no 1T → lembrete intervalo
    if (isHT && total === 0 && tevRaioMand)
      await alertar('over05_ht_lembrete', `🟢 <b>OVER 0.5 — Intervalo 0x0!</b>\n💡 Entrar agora no Over HT!`, 'over05_live');
    // 2T + 0x0 + raio
...
async function processarFimDeJogo(jogoId, estado, hoje) {
  ...
  for (const [stratKey, alertasArr] of Object.entries(estado.msgIds || {})) {
    if (!alertasArr?.length) continue;
    const ultimo = alertasArr[alertasArr.length - 1];
    const stratBase = stratKey.replace(/_entrada$|_0x0$|_raio$|_c[123]$|_perd.*$|_atencao.*$|_reentrada.*$/, '');
    const pLive = pendJogo.find(p => {
      const ps = p.strat.replace(/_live$/, '');
      return ps === stratBase || p.condicao === stratKey;
    });
    const res = pLive?.result;
    const emoji = res === 'green' ? '✅ GREEN' : res === 'red' ? '❌ RED' : '⏳';
    const display = STRAT_DISPLAY[stratBase] || stratKey;
    await editTelegram(ultimo.ids, `${display}\n⚽ ${jogo.mandante} x ${jogo.visitante}\n${emoji} · FT: ${placarFT}`);
  }
...
app.post('/testar-telegram', async (req, res) => {
  await sendTelegram('✅ FUTATS Server v36 funcionando! 🎯');
  res.json({ ok: true });
});

// ── START ─────────────────────────────────────────────────────
app.listen(PORT, async () => {
  console.log(`FUTATS Server v36 na porta ${PORT}`);
  await buscarPreJogo();
  setInterval(buscarPreJogo, 6 * 60 * 60 * 1000);
  setInterval(monitorarLive, 90 * 1000);
  await sendTelegram(
    '🚀 <b>FUTATS Server v36 iniciado!</b>\n' +
    '✅ Fix #1: Gol no Final não dispara no HT\n' +
    '✅ Fix #2: Lay 0x1/1x0 reentrada no 2T\n' +
    '✅ Fix #3: Over 0,5 Gonza — raio dos dois times\n' +
    '✅ Fix #4: Links exchanges corrigidos (url_betfair etc.)\n' +
    '✅ Fix #5: Atualização de placar nas mensagens\n' +
    '✅ Fix #6: Correção Lay não dispara no intervalo'
  );
});
